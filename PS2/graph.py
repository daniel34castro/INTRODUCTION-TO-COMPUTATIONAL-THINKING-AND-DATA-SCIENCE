...
class Digraph(object):
    ...
    def get_edges_for_node(self, node):
        return self.edges[Node(node)]

    def has_node(self, isNode):
        return Node(isNode) in self.nodes
    ...
    def get_path_distance(self, path):
        if len(path)<2:
            # raise ValueError('Path with less than 2 nodes')
            return 0
        dist=0
        for i in range(len(path)-1):
            src=path[i]
            dest=path[i+1]
            for w_edge in self.get_edges_for_node(src):
                if (w_edge.get_destination().get_name()==dest): # get_name() to convert Node to string
                    dist+=w_edge.get_total_distance()
        return dist
    
    def get_path_outdoor_distance(self, path):
        if len(path)<2:
            # raise ValueError('Path with less than 2 nodes')
            return 0
        dist=0
        for i in range(len(path)-1):
            ...
            for w_edge in self.get_edges_for_node(src):
                if (w_edge.get_destination().get_name()==dest): # get_name() to convert Node to string
                    dist+=w_edge.get_outdoor_distance()
        return dist
